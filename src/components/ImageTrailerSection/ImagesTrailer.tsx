...
const ImagesTrailer = () => {
    ...
    const getRandomArbitrary = useCallback((min:number, max:number,step:number) =>{
   // 1. Calculate how many "steps" fit in the range
  const range = (max - min) / step;
  
  // 2. Pick a random number of steps (0 to range)
  const randomSteps = Math.floor(Math.random() * (range + 1));
  
  // 3. Multiply steps by the step size and add the minimum starting point
  return (randomSteps * step) + min;
},[])
...
// console.log("postions",imagesPostions)
